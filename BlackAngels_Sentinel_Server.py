...
def _update_settings(cfg):
    ...
    return {
        ...
        "rollback": bool(raw.get("rollback", False)),
...
def update_config():
    ...
    try:
        ...
        # Per-map gate display names. Merge only the maps/names supplied.
        if isinstance(patch.get("gate_names_by_map"), dict):
            current_maps = cfg.get("gate_names_by_map")
            if not isinstance(current_maps, dict):
                current_maps = {}

            for map_key, names in patch["gate_names_by_map"].items():
                if not isinstance(names, dict):
                    continue
                current_names = current_maps.get(str(map_key))
                if not isinstance(current_names, dict):
                    current_names = {}
                for gate, value in names.items():
                    value = str(value or "").strip()[:32]
                    if value:
                        current_names[str(gate)] = value
                current_maps[str(map_key)] = current_names

            cfg["gate_names_by_map"] = current_maps

        # Per-map normalized gate coordinates.
        # The admin sends the complete table for a map, so replace ONLY that
        # supplied map. All other maps remain untouched in Neon.
        if isinstance(patch.get("gate_positions_by_map"), dict):
            current_maps = cfg.get("gate_positions_by_map")
            if not isinstance(current_maps, dict):
                current_maps = {}
            ...
            for map_key, positions in patch["gate_positions_by_map"].items():
                ...
                current_maps[str(map_key)] = clean

            cfg["gate_positions_by_map"] = current_maps
        ...
        # ONE global Map Area size for every Dino Storm map/user.
    ...
    try:
        ...
        if not client_version.upper().startswith("ADMIN_"):
            ...
            if update_cfg["published"]:
                if update_cfg.get("rollback") and update_cfg["force_update"]:
                    blocked = _version_tuple(client_version) != _version_tuple(update_cfg["latest_version"])
                else:
                    required = (
                    ...
                    blocked = _version_tuple(client_version) < _version_tuple(required)
                if blocked:
                    return jsonify({
                        "ok": False,
